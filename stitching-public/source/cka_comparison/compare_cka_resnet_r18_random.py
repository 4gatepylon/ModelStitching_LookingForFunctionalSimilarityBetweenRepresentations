...
wandb.init(project='cka-random', entity='yaminibansal')
...
path1 = '/n/coxfs01/ybansal/main-exp/results/rep-compare-models/anxwa5og/final_model.pt'
wandb.run.summary[f"layer_ind"] = layer_index
wandb.run.summary[f"layer_ind2"] = layer_index2
wandb.run.summary[f"path1"] = path1
wandb.run.summary[f"path2"] = 'random_init'

model1 = torch.load(path1)
model2 = resnet18k_cifar(k, num_classes).cuda()
