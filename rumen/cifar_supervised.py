...
def main_train_small(args):
    ...
    for combination in combinations:
        ...
        if os.path.exists(fname):
            ...
            model = _resnet(f"resnet_{x}{y}{z}{w}",
                            BasicBlock, combination, False, False)
            model = model.cuda()
...
def main_stitchtrain_small(args):
    ...
    for sender, reciever, stitches, sims, orig_acc1, orig_acc2, name in [
        # Resnet18 -> Resnet18
        (model1, model2, model1_model2_stitches, model1_model2_sims,
         model1_acc, model2_acc, f"{name1}_{name2}"),
        (model1_rand, model2, model1_rand_model2_stitches, model1_rand_model2_sims,
         model1_rand_acc, model2_acc, f"{name1}_rand_{name2}"),
        (model1, model2_rand, model1_model2_rand_stitches, model1_model2_rand_sims,
         model1_acc, model2_rand_acc, f"{name1}_{name2}_rand"),
        (model1_rand, model2_rand, model1_rand_model2_rand_stitches, model1_rand_model2_rand_sims,
        ...
        for i in range(N1 - 1):
            for j in range(1, N2):
                try:
                    ...
                    model = Stitched(sender, reciever,
                                     snd_label, rcv_label, stitch)

                    # Epochs can be changed (large may lead to exploding gradients?)
                    acc, _ = train_loop(
                        model, train_loader, test_loader, epochs=3, parameters=list(stitch.parameters()))
                    acc /= 100.0
                    print(f"\tAccuracy of model is {acc}")
                    print(
